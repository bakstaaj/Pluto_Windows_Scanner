...
# Install/update Windows Pluto SDR Scanner GUI v2.0 Phase 2B Sample Rate Fix into
# the separate GUI folder.
# Default target:
#   ~/sdrdev/pluto_windows_scanner
#
# Run from MSYS2 UCRT64 after extracting this package to:
#   C:\Users\jim\Downloads\PlutoWindowsScannerGuiV2_Phase2B_SampleRateFix
#
# Example:
#   /c/Users/jim/Downloads/PlutoWindowsScannerGuiV2_Phase2B_SampleRateFix/tools/install_windows_scanner_gui_v2_phase2b_sample_rate_fix.sh
...
# Phase 2B safety defaults. Existing configs from the first Phase 2 package may
...
if [[ -f "$TARGET/configs/gui_v2_settings.json" ]]; then
  cp -a "$TARGET/configs/gui_v2_settings.json" "$TARGET/configs/gui_v2_settings.json.bak_phase2b"
...
if [[ -f "$TARGET/configs/bands.csv" ]]; then
  cp -a "$TARGET/configs/bands.csv" "$TARGET/configs/bands.csv.bak_phase2b"
...
Installed Windows Pluto SDR Scanner GUI v2.0 Phase 2B Sample Rate Fix into:
  $TARGET
...
Phase 2B fixes live spectrum UI lockups by throttling renders and using bitmap waterfall drawing.
